...
class TwoDimensionalArrayTests {
	...
	void sortTest() {
		...
		assertArrayEquals(expected, arraySort(array));
	}
	@Test
	void sumTest() {
	...
	private static int [][] arraySort(int[][] ar) {
		return Arrays.stream(ar).sorted((a1 ,a2) -> Integer.compare(Arrays.stream(a1).sum(), Arrays.stream(a2).sum())).toArray(int [][]::new);
	}
}
